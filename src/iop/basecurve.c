...
static const basecurve_preset_t basecurve_presets[] =
{
  ...
  {olympus_alt, "OLYMPUS", "E-M5", 0, 51200,               {{{{0.000000, 0.000000},{0.012097, 0.010322},{0.072581, 0.167742},{0.310484, 0.711291},{0.645161, 0.956855},{1.000000, 1.000000}}}, {6}, {m}}, 1, 0},
...
void init_presets (dt_iop_module_so_t *self)
{
  ...
  for(int k=0; k<basecurve_presets_cnt; k++)
  {
    ...
    dt_gui_presets_update_filter(_(basecurve_presets[k].name), self->op, self->version(), basecurve_presets[k].filter);
  }
  // sql commit
