...
static void _gradient_slider_get_preferred_height (GtkWidget *widget, gint *minimal_height, gint *natural_height);
...
static void
_gradient_slider_get_preferred_width (GtkWidget *widget,
                                      gint *minimal_width,
                                      gint *natural_width)
{
...
static void
_gradient_slider_get_preferred_height (GtkWidget *widget,
                                       gint *minimal_height,
                                       gint *natural_height)
{
  GtkRequisition requisition;
...
static void _gradient_slider_destroy(GtkWidget *widget)
{
  ...
  if(DTGTK_GRADIENT_SLIDER(widget)->timeout_handle) g_source_remove(DTGTK_GRADIENT_SLIDER(widget)->timeout_handle);
  DTGTK_GRADIENT_SLIDER(widget)->timeout_handle = 0;

  if(DTGTK_GRADIENT_SLIDER(widget)->colors)
  {
    g_list_foreach(DTGTK_GRADIENT_SLIDER(widget)->colors, (GFunc)g_free, NULL);
    g_list_free(DTGTK_GRADIENT_SLIDER(widget)->colors);
...
GType dtgtk_gradient_slider_multivalue_get_type()
{
  ...
  if (!dtgtk_gradient_slider_type)
  {
    static const GTypeInfo dtgtk_gradient_slider_info =
    {
      ...
      (GClassInitFunc) _gradient_slider_class_init,
      NULL,           /* class_finalize */
      NULL,           /* class_data */
    ...
    dtgtk_gradient_slider_type = g_type_register_static(GTK_TYPE_WIDGET, "GtkDarktableGradientSlider", &dtgtk_gradient_slider_info, 0);
  }
