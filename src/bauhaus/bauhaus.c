...
dt_bauhaus_popup_button_release(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
      ...
      (dt_get_wtime() - darktable.bauhaus->opentime >= 0.250f)) // default gtk timeout for double-clicks
  {
    ...
    gdk_device_get_position(gdk_device_manager_get_client_pointer(gdk_display_get_device_manager(gdk_display_get_default())), NULL, &x, &y);
...
static gboolean
dt_bauhaus_combobox_scroll(GtkWidget *widget, GdkEventScroll *event, gpointer user_data);
// static gboolean
  ...
  switch(w->type)
  {
    case DT_BAUHAUS_SLIDER:
    {
      ...
      for(int k=0; k<num_scales; k++)
      {
        ...
        cairo_set_source_rgba(cr, darktable.bauhaus->grid, darktable.bauhaus->grid, darktable.bauhaus->grid, d->scale/fabsf(off));
...
void dt_bauhaus_vimkey_exec(const char *input)
{
  ...
  switch(w->type)
  {
    ...
    case DT_BAUHAUS_COMBOBOX:
      ...
      fprintf(stderr, " = %f\n", new_value);
      if(isfinite(new_value))
        dt_bauhaus_combobox_set(GTK_WIDGET(w), new_value);
      break;
    default:
      break;
  }
